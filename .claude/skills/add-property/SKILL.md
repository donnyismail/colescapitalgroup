...
The goal: one new Markdown file in `src/pines/properties/` and the site does everything else
...
## 2. Create the file
`src/pines/properties/<slug>.md` where slug = lowercase name with dashes (e.g. `birch-hollow`).
...
## 3. Photos
- Real photos: save them to `src/pines/assets/uploads/` (any size is fine — the build shrinks
